 ...
 * @author Adriano
 */
public class UpGradeGlosa extends javax.swing.JFrame {

    /**
     * Creates new form UpGradeGlosa
     */
    public UpGradeGlosa() {
        initComponents();
        iniciarComponentes();
    ...
    private void initComponents() {
        ...
        setPreferredSize(new java.awt.Dimension(996, 604));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(UpGradeGlosa.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new UpGradeGlosa().setVisible(true);
            }
        });
    }
    ...
    private DefaultTableModel dtm = new DefaultTableModel(null, new String[]{
        ...
        "Convenio", "Não Conformidade", "Observação", "Status","Valor"});
        ...
        if (list.isEmpty()) {
            ...
            for (SftOutputGlos sftGlos : list) {
                try {
                    ...
                    dtm.setValueAt(sftGlos.getGlosOsNumos(), i, 2);//OS
                    ...
                    dtm.setValueAt(sftGlos.getGlosObObsglos(), i, 7);//seta Observação
                    SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy");
                    dtm.setValueAt(formato.format(sftGlos.getSftCtrlHistoricoList().get(size).getHistDtRegis()), i, 1);//seta data
                    dtm.setValueAt((sftGlos.getFkStat() != null) ? sftGlos.getFkStat()//seta Status se naõ for nulo
                            .getStatusNmDescricao() : null, i, 8);
                    dtm.setValueAt(sftGlos.getGlosVlVlos(), i, 9);
                    i++;
    ...
    private void upload() throws HeadlessException {
        ...
        if (choose.getSelectedFile() != null) {
            ...
            try {
                result = Tradutor.getInstance().getResultList(SftOutputGlos.class, filename);
                new Thread(new UpGradeGlosa.upload()).start();
    ...
    private boolean teste(String texto) {
        List<SftOutputGlos> glosa = dao.getGlosList(texto);
        if (glosa.isEmpty()) {
            return true;
        } else {
            return false;
            ...
            for (String[] columns : result) {
                ...
                if (teste(columns[0])) {
                    ...
                    s.setSftCtrlHistoricoList(
                            Logger.getInstance().glosRegistros("upload", columns[0], 
                                    new ArrayList<SftCtrlHistorico>(),s));
                    ErroMensagens.getInstancia().close();
                    
                    //OS
                    s.setGlosOsNumos(columns[0]);
                    //Unidade
                    try {
                    ...
                    //Convenio
                    try {
                        Convenio get = boxConv.get(codMascara.indexOf(columns[3]));
                        s.setFkConv(get);
                    } catch (ArrayIndexOutOfBoundsException a) {
                        ErroMensagens.getInstancia().Erros(columns[0], "Convenio", columns[3]);
                    }
                    //Atendente
                    try {
                        String aten = "AUTOMATICO";
                        if (codunid == 10) {
                            aten = "FATURAMENTO MERIDIONAL";
                        }
                        if (codunid == 26) {
                            aten = "FATURAMENTO SANTA RITA";
                        }
                        SftInputAten fkAten;
                        fkAten = boxAten.get(codAten.indexOf(aten));
                        s.setFkAten(fkAten);
                    ...
                    //Obs
                    s.setGlosObObsglos(columns[5]);
                    //Não conformidade
                    try {
                        SftInputNcon ncon = boxNcon.get(codNcon.indexOf(columns[4]));
                        s.setFkNcon(ncon);
                    } catch (ArrayIndexOutOfBoundsException a) {
                        ErroMensagens.getInstancia().Erros(columns[0], "Não Conformidade", columns[4]);
                    }
                    //Data
                    Date date = LeData.LeBrasil(columns[6],"-");
                    s.getSftCtrlHistoricoList().get(0).setHistDtRegis(date);
                    s.setGlosDtRegis(date);
                    //Valor
                    try{
                        s.setGlosVlVlos(Double.valueOf(columns[7]));
                    }catch(NumberFormatException e){
                        ErroMensagens.getInstancia().Erros(columns[0], "Valor", columns[7]);
                    }
